...
export const normalizeSvgToCanvas = (svgString: string, targetSize = 1000): string => {
...
export const getGlyphBounds = (svgString: string | undefined): GlyphBounds => {
  const fallback: GlyphBounds = { minX: 0, maxX: 100, minY: 0, maxY: 100, gridWidth: 100, gridHeight: 100, isEmpty: true };
  if (!svgString || !svgString.trim()) return fallback;

  const vbMatch = svgString.match(/viewBox=["']0\s+0\s+([\d.]+)\s+([\d.]+)["']/i);
  const gridWidth = vbMatch ? parseFloat(vbMatch[1]) : 100;
  const gridHeight = vbMatch ? parseFloat(vbMatch[2]) : 100;
...
export const computeGlyphAdvance = (
  ...
  const gw = bounds.gridWidth || 100;
  const gh = bounds.gridHeight || 100;
  const unitsPerGrid = 1000 / gw;
  const glyphScaleFactor = (art.scale / 100) * (700 / Math.max(gw, gh, 1));
  const desiredLSB = 70;

  if (exportSpacingMode === "monospace") {
    return {
      advanceWidth: Math.round(unitsPerGrid * gw),
      xShift: 0,
    };
  }

  if (bounds.isEmpty) {
    return { advanceWidth: 650, xShift: 0 };
  }

  const contentWidthOTF = (bounds.maxX - bounds.minX) * glyphScaleFactor;
  const advanceWidth = Math.round(desiredLSB + contentWidthOTF + desiredLSB) + (art.kerning ?? 0) * 8;
...
export const applyAutoKerning = (
  ...
  glyphList.forEach((glyph) => {
    ...
    const gridW = bounds.gridWidth || 100;
    const rightBearing = gridW - bounds.maxX;
    const leftBearing = bounds.minX;
    const centerOffset = (leftBearing - rightBearing) / gridW;

    let kerning = Math.round(centerOffset * 12);

    if (contentWidth < gridW * 0.2) {
      kerning -= 2;
    } else if (contentWidth > gridW * 0.7) {
      kerning += 2;
    }

    kerning = Math.max(-10, Math.min(10, kerning));
    next[glyph] = { ...item, kerning };
  });
...
const neatForGlyph = (glyph: string, art: GlyphArt): GlyphArt => {
  // First normalize the SVG to 1000x1000 canvas coordinates
  const normalizedSvg = normalizeSvgToCanvas(art.svg, 1000);
  ...
  // Standard font grid positions (in 1000-unit UPM coordinate space)
  const ascent = 180;    // top of uppercase letters
  const capHeight = 260; // top of lowercase letters  
  const baseline = 740;  // where letters sit
  const descent = 880;   // bottom of descenders like g, j, p, q, y
  ...
  const tx = (1000 - newW) / 2 - bounds.minX * finalScale;
  ...
  const rawSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000"><g transform="translate(${tx}, ${ty}) scale(${finalScale})">${contentMatch[1]}</g></svg>`;
